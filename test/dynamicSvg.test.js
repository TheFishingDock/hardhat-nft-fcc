...
const priceFeedsAddress = !developmentChains.includes(network.name)
    ? describe.skip
    : describe("Dynamic SVG NFT Unit Tests", function () {
          ...
          describe("constructor", () => {
              it("sets starting values correctly", async function () {
                  const lowSVG = await dynamicSvgNft.svgToImageURI(lowTokenUri)
                  const highSVG = await dynamicSvgNft.svgToImageURI(highTokenUri)
                  const priceFeed = await mockV3Aggregator.address
